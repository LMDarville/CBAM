     ...
     $(document).ready(function() {
         ...
         $(function() {
             ...
             $("#tabs").bind("tabsselect", function(event, ui) {
                 ...
                 //                 if ($tabs.tabs('option', 'selected') == 0) { //left 1st tab, clear votes bottom 2/3, update status
                 //                     var urli = '<%=Url.Action("ClearVotes_bottomTwoThirds", "Steps")%>';
                 ...
                 //                     //jquery post and ajax call don't seem to consistently work correctly
                 //                     jQuery.ajax({
                 //                         url: urli,
                 //                         type: "POST",
                 //                         success: function() { console.log('successful .ajax') },
                 //                         error: function() { console.log('error on .ajax') }
                 //                     })
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
  ...
  <% Html.RenderAction("ProgressBar"); %>
  
  <div>  <%=Html.Encode(Model.nextStepToComplete.Step1)%> <%=Html.Encode(Model.nextStepToComplete.Description)%> </div>
  <div id="next" visible="false"> <%=Html.Encode(Model.nextStepToComplete.Step1)%> </div>
  
  <div><span id="results">hi</span> </div>
        ...
        <div id="tab-<%= i %>">
            ...
            <% switch(i) { %>
                ...
                <% case 2: %>   <!------------ Step 3 ------------>
                     ...
                     <div>
                        <%if (Model.nextStepToComplete.Step1 >= i+1) //ensure prior steps are complete 
                          { %> <%= Html.ActionLink("Edit", "EditVotes", "Votes")%>
                        <%; }  
                         else { %> <span class="ui-state-error"> Must Complete Step <%=Html.Encode(Model.nextStepToComplete.Step1)%>           
                                    <a style="color: blue" href="Scenario#<%=Html.Encode(Model.nextStepToComplete.TabRef)%>" class="text-link"><%=Html.Encode(Model.nextStepToComplete.Description)%> </a>
                ...
                <% default: %>
                    ...
                    Scenario: <%= i+1 %>. <%=Html.Encode(Model.Steps[i].Description)%>
                <% break; %>
            <% }; %>
